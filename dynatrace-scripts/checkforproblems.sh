...
output=$(curl -k -H "Content-Type: application/json" -H "Authorization: Api-Token ${DT_TOKEN}" -X GET ${DT_URL}/api/v1/problem/status)
echo 'Received server response:' $output
